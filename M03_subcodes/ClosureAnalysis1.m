...
%% filtering the curve
    ...
    common_time = (linspace(max([min(time_P),min(time_w)]), min([max(time_P),max(time_w)]), 1000))';
    ...
    cutoff_freq = 15; % Adjust cutoff frequency as needed
    fs = 500; % Sampling frequency (assumed, adjust as appropriate)
...
%% Leakoff asymptote calculation at late time
    CCCC=[];
    aaa=1;
    for i22=102:1:numel(common_time)
