function  t_tsqrt_G_P_w = ClosureAnalysis1(t_P_org,t_w_org,t_reccession,w_reccesion)
...
%% filtering the curve
    ...
    common_time = (linspace(max([min(time_P),min(time_w)]), min([max(time_P),max(time_w)]), 4000))';
    ...
    cutoff_freq = 20; % Adjust cutoff frequency as needed
    fs = 4000; % Sampling frequency (assumed, adjust as appropriate)
...
%% Leakoff asymptote calculation at late time
    ...
    for i22=402:1:numel(common_time)
        ...
        p1 = polyfit(t_r(end-400:end),w_r(end-400:end),1);
