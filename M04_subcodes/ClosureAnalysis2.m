function  t_tsqrt_G_P_w = ClosureAnalysis2(t_P_org,t_w_org,t_reccession,w_reccesion)
...
%% filtering the curve
    ...
    common_time = (linspace(max([min(time_P),min(time_w)]), min([max(time_P),max(time_w)]), 5000))';
    ...
    fs = 5000; % Sampling frequency (assumed, adjust as appropriate)
...
%% Leakoff asymptote calculation at late time
    ...
    for i22=502:1:numel(common_time)
