function  t_tsqrt_G_P_w = ClosureAnalysis3(t_P_org,t_w_org,t_reccession,w_reccesion)
...
%% filtering the curve
    ...
    common_time = (linspace(max([min(time_P),min(time_w)]), min([max(time_P),max(time_w)]), 10000))';
    ...
    cutoff_freq = 70; % Adjust cutoff frequency as needed
    fs = 10000; % Sampling frequency (assumed, adjust as appropriate)
...
%% sunset solution at selected point
    ...
    p2=polyfit(sqrt(t_r(end-4:end)),p_r(end-4:end),1);
...
%% Late time - sunset solution shape
    ...
    p1=polyfit(t_r(end-500:end),w_r(end-500:end),1);
    xx=min(t_r):0.001:max(t_r);
    yy=polyval(p1,xx);
    ...
    p2=polyfit(sqrt(t_r(end-20:end)),p_r(end-20:end),1);
    xx=min(t_r):0.0001:max(t_r);
    yy=polyval(p2,sqrt(xx));
...
%% Leakoff asymptote calculation at late time
    ...
    for i22=1002:1:numel(common_time)
        ...
        p1 = polyfit(t_r(end-1000:end),w_r(end-1000:end),1);
