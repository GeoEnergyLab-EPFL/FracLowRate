function  t_tsqrt_G_P_w = ClosureAnalysis5(t_P_org,t_w_org,t_reccession,w_reccesion)
...
%% filtering the curve
    ...
    common_time = (linspace(max([min(time_P),min(time_w)]), min([max(time_P),max(time_w)]), 3000))';
    ...
    fs = 3000; % Sampling frequency (assumed, adjust as appropriate)
